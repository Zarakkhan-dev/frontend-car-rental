...
function CarBookingList() {
  const { data: apiBookingData, error, isLoading } = useGetAllBookingsQuery();
  const [carBookingDetails, setCarBookingDetails] = useState([]);

  // Initialize booking details from API response
  useEffect(() => {
    if (apiBookingData) {
      setCarBookingDetails(apiBookingData);
    }
  }, [apiBookingData]);
    ...
    <div className="p-6 bg-gray-100 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left border border-gray-200 shadow-md text-nowrap">
          ...
          <tbody>
            {carBookingDetails && carBookingDetails.length > 0 ? (
              carBookingDetails.map((booking, index) => (
                <tr
                  key={index}
                  className={`${
                ...
                >
                  ...
                  <td className="px-4 py-3 flex gap-3 justify-center items-center border-t border-gray-200">
                    <Link
                      to={`/CarBookingUpdate/${booking.car_id}`}
                      className="text-gray-500 hover:text-gray-700"
                    >
